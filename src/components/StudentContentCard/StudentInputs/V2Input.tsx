...
import { V2QuickOptionsManager } from "../../../utils/inputAssistant";
...
const V2InputItem = memo(({
  ...
  const handleInputBlur = useCallback((value: string) => {
    ...
    V2QuickOptionsManager.addCustomOption(item.itemKey, value.trim());
  }, [item.itemKey]);
 ...
 * V2版本输入组件 - 多字段输入
 */
const V2Input = memo(({ index, disabled, totalStudents, sendWarning }: V2InputProps) => {
