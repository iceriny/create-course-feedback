...
class API {
  ...
  // 节流状态监听器
  private static throttleListeners: ((
    isThrottled: boolean,
    message: string,
  ) => void)[] = [];
  ...
  static tokenReady() {
    return API.token !== undefined && API.token !== "" && API.token !== null;
  }
  ...
  // 添加节流状态监听器
  static addThrottleListener(
    listener: (isThrottled: boolean, message: string) => void,
  ) {
    API.throttleListeners.push(listener);
    return () => {
      API.throttleListeners = API.throttleListeners.filter(
        (l) => l !== listener,
      );
    };
  }
  ...
  // 通知所有监听器节流状态
  private static notifyThrottleListeners(
    isThrottled: boolean,
    message: string,
  ) {
    API.throttleListeners.forEach((listener) => listener(isThrottled, message));
  }
    ...
    try {
      ...
      if (throttleStatus.isThrottled) {
        ...
        if (throttleStatus.shouldNotify) {
          API.notifyThrottleListeners(true, throttleStatus.message);
        }

        // 等待合适的时间后重试
        const waitTime =
          throttleStatus.requestsThisSecond >= API.MAX_REQUESTS_PER_SECOND
            ? 1000
            : 5000;
        setTimeout(() => {
      ...
      // 通知UI取消节流提醒
      API.notifyThrottleListeners(false, "");
  ...
  private async _sendGeminiMessage(
    ...
    const geminiRequestBody = {
      ...
      generationConfig: {
        temperature: this.messages.temperature,
        topP: this.messages.top_p,
        topK: this.messages.top_k,
        maxOutputTokens: this.messages.max_tokens,
      },
    };
      ...
      fetch(apiUrl)
        ...
        .then((response: GeminiModelResponse) => {
          ...
          if (response.models && response.models.length > 0) {
            API.model_list = response.models.map((item) => item.name);
          } else {
    ...
    fetch(providerConfig.modelListUrl, options)
      ...
      .catch((err) => {
        ...
        switch (currentProvider) {
          case "openai":
            API.model_list = ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo"];
            break;
          case "deepseek":
